# /home/robot/perceptron_ev3.py
# Multiclass Perceptron runtime (one-vs-rest). Python 3.5 compatible.
import os
...
class PerceptronPortable:
    def __init__(self, path=None, mode="calib"):
        """
        mode='calib' -> normalize using runtime white/black to [0,1],
                        then 6D features [nL,nC,nR,nL-nC,nR-nC,nL-nR].
        """
        base = os.path.dirname(os.path.abspath(__file__))
        model_dir = os.path.join(base, "export_perc") if path is None else os.path.abspath(path)

        fW   = os.path.join(model_dir, "perc_W.csv")
        fb   = os.path.join(model_dir, "perc_b.csv")
        fcls = os.path.join(model_dir, "perc_classes.csv")
        ...
        if len(self.W)==0:
            raise ValueError("perc_W.csv is empty")
    ...
    def _score_row(self, w, b, x):
        s = 0.0
        for i in range(len(x)): s += w[i] * x[i]
        return s + b
    ...
    def predict(self, L=None, C=None, R=None):
        ...
        for ci in range(self.C):
            s = self._score_row(self.W[ci], self.b[ci], x)
            if (best_s is None) or (s > best_s):
                best_s = s; best_i = ci
        return self.classes[best_i]

    def predict_with_scores(self, L=None, C=None, R=None):
        if L is None or C is None or R is None:
            raise ValueError("Pass raw L, C, R")
        x = self._features_from_raw(L, C, R)
        scores = []
        ...
        for ci in range(self.C):
            ...
            if (best_s is None) or (s > best_s):
                best_s = s; best_i = ci
        return self.classes[best_i], scores, x
